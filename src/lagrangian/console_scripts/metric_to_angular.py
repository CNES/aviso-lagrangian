...
# with core. If not, see <http://www.gnu.org/licenses/>.
import argparse
import fnmatch
import os
...
def usage():
    ...
    parser.add_argument('dst',
                        ...
                        'converted',
                        type=directory)
    parser.add_argument('u', help='NetCDF variable containing the U component')
...
def get_axis(variables):
    ...
    for name, variable in variables.items():
        if variable.ndim == 1 and variable.dimensions[0] == name:
            if lagrangian.axis.LongitudeUnit()(variable.units.encode('ASCII')):
                x = name
            elif lagrangian.axis.LatitudeUnit()(
...
def convert(x, y, u, v):
    ...
    y1 = numpy.degrees(
        numpy.arcsin(zc / numpy.sqrt(xc * xc + yc * yc + zc * zc)))
